...
AAI_Enemy_Controller_1::AAI_Enemy_Controller_1()
{
	...
	GetPerceptionComponent()->OnPerceptionUpdated.AddDynamic(this, &AAI_Enemy_Controller_1::OnPawnDetected);
	GetPerceptionComponent()->ConfigureSense(*SightConfig);
}

ETeamAttitude::Type AAI_Enemy_Controller_1::GetTeamAttitudeTowards(const AActor& Other) const
{
...
void AAI_Enemy_Controller_1::BeginPlay()
{
	Super::BeginPlay();
...
void AAI_Enemy_Controller_1::OnPossess(APawn* OnPawn)
{
	// Can posses a pawn as a controller
	Super::OnPossess(OnPawn);
}

void AAI_Enemy_Controller_1::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

	AAI_Bot_Enemy_1* Actor = Cast<AAI_Bot_Enemy_1>(GetPawn());
	...
	if (bIsPlayerDetected == true)
	{
		...
		MoveToActor(Player, 3.0f);
	}
}

FRotator AAI_Enemy_Controller_1::GetControlRotation() const
{
	if (GetPawn() == nullptr)
	{
...
void AAI_Enemy_Controller_1::OnPawnDetected(const TArray<AActor*>& DetectedPawns)
{
